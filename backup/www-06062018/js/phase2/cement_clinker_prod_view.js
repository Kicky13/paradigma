...
function prod(opco, sd, selectedLastYear){
    var Url = $.getJSON(url_ol + '/api/index.php/fin_cost/get_data_mview?date='+sd+'&company='+opco, function(data){
        var realCement = data['s'+opco][0].act_bulan_ini.cement;
        var realCementLastyear = data['s'+opco][0].act_tahun_lalu.cement;
        var realClinker = data['s'+opco][0].act_bulan_ini.clinker;
        var realClinkerLastyear = data['s'+opco][0].act_tahun_lalu.clinker;
        var rkapCement = data['s'+opco][0].rkap_bulan_ini.cement;
        var rkapClinker = data['s'+opco][0].rkap_bulan_ini.clinker;
        var percentCement = (realCement/rkapCement)*100;
        var percentClinker = (realClinker/rkapClinker)*100;
        var yoyCement = (realCement/realCementLastyear)*100;
        var yoyClinker = (realClinker/realClinkerLastyear)*100;
        ...
        $('#REALcementLastyear').html(setFormat(realCementLastyear));
        $('#REALclinkerLastyear').html(setFormat(realClinkerLastyear));
        $('#perc3a').html(setFormat(percentCement));
        $('#perc4a').html(setFormat(percentClinker));
        $('#yoy_current_Cement').html(setFormat(yoyCement.toFixed(2)));
        $('#yoy_current_Clinker').html(setFormat(yoyClinker.toFixed(2)));
    });
};

function opcoGroupNew(opco, sd, selectedLastYear) {
    prod(opco, sd, selectedLastYear);
}

function opcoGroupNewbu(opco, sd, selectedLastYear) {
    // console.log(opco);
    // console.log(selectedLastYear);
    /* COMPARISON */
    var urlsmi = url_ol + '/api/index.php/fin_cost/get_data_mview?date=' + sd + '&company=smi';
    ...
    var urlsmi2 = url_ol + '/api/index.php/fin_cost/get_data_mview?date=' + selectedLastYear + '&company=smi';
    ...
    $.when(
        ...
        $.getJSON(url40000),
        $.getJSON(url70000)
    ).done(function (resultsmi, result3000, result4000, result7000, resultsmi, result30000, result40000, result70000) {
