...
class DailyMs extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('m_daily_ms');
    }
 
    function index() {
        //$data = array('title' => 'Daily Market Share');
...
//            $ms = round(str_replace(',', '.', $row['RKAP_MS']));
            ...
            $realisasi = number_format(round(str_replace(',', '.', $row["HASIL"])));
            $demand = number_format(round(str_replace(',', '.', $row["QTY"])));
...
//                    . "<div>MS : <span class='pull-right'>" . $realrkap . " %</span></div>"
//                    . "<div>RKAP : </div>{br}"
...
//                    . "</div>";
    ...
    function getDetail($org, $prov, $tahun, $bulan) {
        $data = $this->DailyMs_model->getDetail($prov, $tahun, $bulan);
        $no = 1;
        $table = '';
        $prov = '';
        $qtybulansmi = 0;
    ...
    function getSummary($tahun, $bulan) {
        $data = $this->DailyMs_model->getSummary($tahun, $bulan);
    ...
    function getDetail2($org, $prov, $tahun, $bulan) {
        $data = $this->DailyMs_model->getDetail2($prov, $tahun, $bulan);
        $no = 1;
        $table = '';
        $prov = '';
        foreach ($data as $row) {
    ...
    function getUpdateDate() {
        $data = $this->DailyMs_model->updateDate();
        echo json_encode($data);
    }

    function rkapsi($tahun) {
        $data = $this->DailyMs_model->getRkap($tahun);
        $target = 0;
    ...
    function coba() {
        $data = $this->DailyMs_model->getSummary('2016', '01');
        echo '<pre>';
    ...
    function getChartGrowth($org) {
        ...
        foreach ($data as $key => $value) {
            ...
            foreach ($value as $row) {
                ...
                $realkumtahunini = str_replace(',', '.', $row['REAL_TAHUNINI_KUM']);
                $realkumtahun = str_replace(',', '.', $row['REAL_TAHUN_KUM']);
				$rkapms = str_replace(',',',', $row['MS']);
                //$rkap = round(str_replace(',', '.', $row["RKAP"]), 2);
                if ($row['KODE_PERUSAHAAN'] == '102' || $row['KODE_PERUSAHAAN'] == '110' || $row['KODE_PERUSAHAAN'] == '112' || $row['KODE_PERUSAHAAN'] == '114') {
                    $realbulaniniSMI += $realbulanini;
                    $realbulankemarinSMI += $realbulankemarin;
                    $realtahunkemarinSMI += $realtahunkemarin;
                    $realkumtahuniniSMI += $realkumtahunini;
    ...
    function marketVolume($kodeperusahaan){
        ...
        $data = $this->DailyMs_model->marketVolume($awal,$akhir,$tipe);
        $labels = array();
    ...
    function dataHistory($awal,$akhir){
        $data = $this->DailyMs_model->dataHistory($awal,$akhir);
	...
	function getDataSemua(){
		$result_perusahaan = $this->m_daily_ms->getdetailSemua();
		$i = 1;
        $return[$i] = array();
        
        foreach ($result_perusahaan as $key=>$value){
			
			$rkap_ms = $this->getPersen($value["MARKETSHARE"],$value['RKAP_MS']);
            $return[$i] = array(
	...
	function getDataSemuaSG(){
		$result_perusahaan = $this->m_daily_ms->getDetailSG();
		$i = 1;
        $return[$i] = array();
	...
	function getDataSemuaSP(){
		$result_perusahaan = $this->m_daily_ms->getDetailSP();
		$i = 1;
        $return[$i] = array();
	...
	function getDataSemuaST(){
		$result_perusahaan = $this->m_daily_ms->getDetailST();
		$i = 1;
        $return[$i] = array();
	...
	function getDataSemuaTLCC(){
		$result_perusahaan = $this->m_daily_ms->getDetailTLCC();
		$i = 1;
        $return[$i] = array();
