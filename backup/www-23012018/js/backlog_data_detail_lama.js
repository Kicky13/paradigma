 ...
 $.ajax({
  ...
  success: function(data){
   ...
   if (opco == 'padang' || opco == 'tonasa' || opco == 'tlcc'){
    ...
    // Total Backlog
    total = Number(((total_osp) / (total_all)) * 100).toFixed(1);  
   ...
   $('#osno').html(total_osno);
   $('#nopr').html(total_nopr);
   $('#noco').html(total_noco);
   $('#dlfl').html(total_dlfl);
   function total_permonth(periode){
    ...
    if (opco == 'padang' || opco == 'tonasa' || opco == 'tlcc'){
     if (obj[data_src][periode] != undefined){
      ...
      total_backlog_perbulan = Number((total_osp_perbulan / total_all_perbulan)).toFixed(2);
     }else{
      total_backlog_perbulan = 0;
     }
    }else if (opco == 'gresik'){
     if (obj[data_src1][periode] != undefined && obj[data_src2][periode] != undefined && obj[data_src3][periode] != undefined){
