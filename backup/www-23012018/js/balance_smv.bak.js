...
function balance_data(bulan, opco, year){
  ...
  var nowdate = moment();
	...
	$('#tag_last_balance').html(month[tag_last_bln]);
  ...
  // if (sessionStorage.getItem('balance_data'+bulan)==null) {
      ...
      var dataJson  = JSON.parse(data);
    	...
    	console.log(dataJson);
  	   ...
  	   stop_waitMe('.headsix add_fix', 'ios');
...
function nextPage(month, year){
  ...
  window.location.href = "balance_s_dtmv.html";
}
function refresh(html){
     sessionStorage.removeItem('bs_dt');
     window.location.href = html+"mv.html";
...
function company_balance(bulan){
    ...
    $.post(url_src+'par4digma/api/index.php/balance_report/company_balance?bulan='+bulan, function(data){
        var dataJson  = JSON.parse(data);
