...
function cost_data_detail(bulan, opco, yearnow){
    ...
    if (sessionStorage.getItem('cost-thn')!=null) {
      var data = sessionStorage.getItem('cost-thn');
...
// DETAIL
		...
		$('#tot_pcost_nw').html('Rp. '+setFormat(tot_pcost_nwx, 0)+' M');
...
// cogs PAST
		...
		$('#tot_cogs_nwu').html('Rp. '+setFormat(tot_cogs_nwx, 0)+' M');
...
// General PAST
		...
		$('#tot_gen_nwu').html('Rp. '+setFormat(tot_gen_nwx, 0)+' M');
...
// sales now
		var sell_dam_now = Number(dataJson.selling_marketing["0"].bulan_ini["Depl. Deprec. and Amortization"])/1000000;
				...
				var sell_tax_now = Number(dataJson.selling_marketing["0"].bulan_ini["Taxes and insurance"])/1000000;
...
// sales PAST
		...
		var sell_tax_past = Number(dataJson.selling_marketing["0"].bulan_ini_lyear["Taxes and insurance"])/1000000;
...
console.log(typeof(tot_sell_nw));
		...
		$('#tot_sell_nwu').html('Rp. '+setFormat(tot_sell_nw, 0)+' M');
