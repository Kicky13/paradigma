    ...
    public function wilayah_detail()
    {
        ...
        $real_propinsi=$this->db->query("SELECT
            ...
            if($row['RKAP']==0){
                ...
                $PERCENT_REAL=($real_propinsi->NTGEW/$row['RKAP'])*100;
            }
            
            $data[$wilayah][]=array('DISTRIK'=>$row['DISTRIK'],'REAL'=>$real_propinsi->NTGEW,'RKAP'=>$row['RKAP'],'PROGNOSA'=>$row['PROGNOSA'],'PERCENT_REAL'=>$PERCENT_REAL);
        }
