...
class F_performance extends CI_Controller{
    ...
    function performance(){
        $filter['bulan'] = (empty($_GET['bulan']) ? date('m') : $_GET['bulan']);
        $filter['company'] = (empty($_GET['company']) ? 'ALL' : $_GET['company']);
        $paramCompany = $filter['company'];
        $param=array(
            "company"=>trim($paramCompany),
            "now"=>trim(date('Y').'.'.$filter['bulan']),
            "yesterday"=>trim(date('Y').'.'.sprintf("%'.02d\n", $filter['bulan']-1)),
            "last_year"=>trim((date('Y')-1).'.'.$filter['bulan']),
            "prev_month"=>trim(date('Y').'.'.sprintf("%'.02d\n", $filter['bulan']-1)),
            "last_year_prev_month"=>trim((date('Y')-1).'.'.sprintf("%'.02d\n", $filter['bulan']-1))
        );
       $paramPembanding = array(
           "company"=>$paramCompany,
           "now"=>(date('Y')-1).'.'.$filter['bulan'],
           "yesterday"=> (date('Y')-1).'.'.sprintf("%'.02d\n", $filter['bulan']-1)
       );
