...
class Cash_flow_st extends CI_Controller {
	...
	$bulan = (empty($_GET['bulan']) ? date('m') : $_GET['bulan']);
        ...
        $bulan_sekarang=$tahun.'-'.$bulan;
     	if($bulan==date('m')){
     		...
     		$cek_data=$this->db->query("SELECT
										...
										FROM
											CF_DAILY
										WHERE NILAI <> 0 AND DATE_FORMAT(DATE,'%Y-%m') = '".$bulan_sekarang."'")->row();
     	}
		...
		$saldo=$this->db->query("SELECT SUM(NILAI) AS NILAI FROM CF_ENDING_BALANCE WHERE DATE = '$tanggal'")->row();

		$begin_balance=$this->db->query("SELECT MIN(NILAI) AS NILAI FROM CF_DAILY WHERE DATE_FORMAT(DATE,'%Y-%m-%d')='".date('Y-m',strtotime($tanggal))."-01' AND KET = 'Saldo Awal'")->row();
    ...
    $tanggal=$cek_data->tanggal;
		// $data=[];
		$sql=$this->db->query("SELECT DATE,SUM(NILAI) AS NILAI FROM CF_ENDING_BALANCE WHERE DATE_FORMAT(DATE,'%m')='".date('m',strtotime($tanggal))."' AND NILAI <> 0 GROUP BY DATE");
     	...
     	$tanggal=$cek_data->tanggal;
            ...
            $in=$this->db->query("SELECT
                                                                            ...
                                                                            WHERE
                                                                                    KET IN ('- Hasil Penjualan','- Pendapatan Bunga','- Pendapatan Lain','- Kompensasi')
                                                                            AND
                                                                                    DATE_FORMAT(DATE,'%Y-%m')='".date('Y-m',strtotime($tanggal))."'
                                                                                    GROUP BY DATE");
