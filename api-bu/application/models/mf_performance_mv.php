 ...
 */
class mf_performance_mv extends CI_Model {
    ...
    public function get_labarugi($com, $date, $category, $title){
      $db = $this->load->database('default3',true);
      $sql = "
        SELECT * FROM(SELECT 
        title, sum(val) VAL
        FROM MV_LABA_RUGI_V1
        WHERE
          TITLE IN ( 'PL_OA', 'PL_VLP', 'PL_BUA', 'PL_BPE')
        AND FISCAL_YEAR_PERIOD IN ( '$date') 
        and company IN $com
        AND CATEGORY = '$category'
        GROUP BY title)
        PIVOT (
          SUM(VAL) FOR (TITLE) IN ('PL_OA' as OA, 'PL_VLP' AS VLP, 'PL_BUA' AS BUA, 'PL_BPE' AS BPE)
        )
      ";
    ...
    public function get_data_upto($com,$comParam, $date_now, $date_start, $category){


      if ($com == '7000') {
        # code...
        $sqlTitle = "('PL_VLP','PL_OA','PL_BUA','PL_BPE')";
        
      }else if ($com == '3000' || $com == '4000') {
        # code...
         $sqlTitle = "('PL_VLP')";
      }else{
        $sqlTitle = "('PL_VLP','PL_OA','PL_BUA','PL_BPE')";
      ...
      $sql = "
        ...
        WHERE
          TANGGAL BETWEEN TO_DATE ('$date_start', 'YYYY.MM')
        AND TO_DATE ('$date_now', 'YYYY.MM')
        GROUP BY
          title
        UNION
          SELECT
          ...
          FROM
            MV_LABA_RUGI_V1
          WHERE
            FISCAL_YEAR_PERIOD LIKE '$date_now'
          AND company IN $comParam
          AND CATEGORY = '$category'
          AND TITLE NOT IN $sqlTitle
          GROUP BY
            title
    ...
    public function get_labarugi2($com, $date_current, $date_last, $category){
          ...
          FROM
            (
              ...
              FROM
                MV_LABA_RUGI_V1
              WHERE
                FISCAL_YEAR_PERIOD IN ('$date_current', '$date_last')
              AND company IN $com 
              AND CATEGORY = '$category'
              GROUP BY
                title,
              ...
              SUM (VAL) FOR (FISCAL_YEAR_PERIOD) IN (
                '$date_current' AS NOW,
                '$date_last' AS LAST
              )
            )
 ...
 function mget_ebitda_upto($param){
  ...
  function mget_ebitda($param){
   ...
   if($param['company'] == 'ALL'){
       ...
       $paramCompany = "AND COMPANY IN {$param['company']}";
       $paramCompanyLast = "AND COMPANY IN {$param['company']}";
   }   
   ...
   for ($x=1;$x<=intval($now);$x++){
    $formated = "'".$param['tahun'].'.'.$x."'";
    if ($x < 10){
     $formated = "'".$param['tahun'].'.0'.$x."'";
    }
    array_push($period, $formated);    
   } 
   for ($x=1;$x<=intval($now);$x++){
    $formated = "'".($param['tahun']-1).'.'.$x."'";
    if ($x < 10){
     $formated = "'".($param['tahun']-1).'.0'.$x."'";
    }
    array_push($period_yes, $formated);    
   }   
   $sql = "SELECT
            NVL (EBIT_T.TOTAL, 0) ebit_target,
            NVL (EBIT_A.TOTAL, 0) ebit_actual,
            NVL (EBIT_ALY.TOTAL, 0) ebit_actual_lyear,
            NVL (EBIT_TUP.TOTAL, 0) ebit_target_up,
            NVL (EBIT_AUP.TOTAL, 0) ebit_actual_up,
            NVL (EBIT_AUPLY.TOTAL, 0) ebit_actual_lyear_up
          FROM
            (
              SELECT
                SUM (amount) total
            ...
            (
              ...
              AND GL_ACCOUNT = 'PL_E'
              AND CURRENCY = 'LC' $paramCompanyLast
            ) ebit_aly,
            (
              SELECT
                SUM (amount) total
  ...
  function mget_volume($param){
   ...
   if($param['company'] == 'ALL'){
       ...
       $paramCompany = "COMPANY IN {$param['company']}";
       $paramCompanyLast = "COMPANY IN {$param['company']}";
  ...
  function mget_bruto($param){
   ...
   $sql = "SELECT
            NVL (BT.BRUTO, 0) bruto_target,
            NVL (BA.BRUTO, 0) bruto_actual,
            NVL (BALY.BRUTO, 0) bruto_actual_lyear,
            NVL (BTUP.BRUTO, 0) bruto_target_up,
            NVL (BAUP.BRUTO, 0) bruto_actual_up,
            NVL (BALYUP.BRUTO, 0) bruto_actual_lyear_up
          FROM
            (
              SELECT
                SUM (AMOUNT) AS bruto
            ...
            (
              ...
              WHERE
                CATEGORY = 'ACT'
              AND FISCAL_YEAR_PERIOD = '{$param['last_year']}'
              AND GL_ACCOUNT = 'PL_HPB'
              AND CURRENCY = 'LC'
              AND $paramCompanyLast
            ) baly,
            (
              SELECT
                SUM (AMOUNT) AS bruto
  ...
  function mget_cost($param){
   ...
   $sql = "SELECT
              NVL (BU_T.BEBAN + BP_T.BEBAN, 0) cost_target,
              NVL (BU_A.BEBAN + BP_A.BEBAN, 0) cost_actual,
              NVL (
                BU_ALY.BEBAN + BP_ALY.BEBAN,
                0
              ) cost_actual_lyear,
              NVL (
                BU_TUP.BEBAN + BP_TUP.BEBAN,
                0
              ) cost_target_up,
              NVL (
                BU_AUP.BEBAN + BP_AUP.BEBAN,
                0
              ) cost_actual_up,
              NVL (
                BU_ALYUP.BEBAN + BP_ALYUP.BEBAN,
                0
              ) cost_actual_lyear_up
            FROM
              (
                SELECT
                  SUM (AMOUNT) AS beban
  ...
  function mget_revenue($param){
   ...
   $sql = "SELECT
            NVL (LBT.LABA, 0) laba_bruto_target,
            NVL (LBA.LABA, 0) laba_bruto_actual,
            NVL (LBTUP.LABA, 0) laba_bruto_target_up,
            NVL (LBAUP.LABA, 0) laba_bruto_actual_up,
            NVL (LNT.LABA, 0) laba_netto_target,
            NVL (LNA.LABA, 0) laba_netto_actual,
            NVL (LNTUP.LABA, 0) laba_netto_target_up,
            NVL (LNAUP.LABA, 0) laba_netto_actual_up
          FROM
            (
              SELECT
                SUM (AMOUNT) AS laba
            ...
            (
              ...
              WHERE
                CATEGORY = 'BUD'
              AND FISCAL_YEAR_PERIOD = '{$param['now']}'
              AND GL_ACCOUNT IN ('PL_OA', 'PL_HPB')
              AND CURRENCY = 'LC'
              AND $paramCompany
            ) lnt,
            (
