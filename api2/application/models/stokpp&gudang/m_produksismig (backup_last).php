...
class M_produksismig extends CI_Model {
   ...
   //              )TB3 ON TB1.ORG = TB3.ORG");
        ...
        foreach ($sql_teraks6000->result_array() as $rowID) {
          ...
          $prod_smig['s'.$org]=array('prognose'=>$rowID['PROGNOSE'],
                        'vol_prog'=>$smig_vol_prog6000,
                        'realisasi'=>$smig_realisasi6000);
        }
      ...
      $temp_semen['rkap'] = $total_rkapsemen;
        ...
        $smig_vol_progsemen6000=0;
        $smig_realisasisemen6000=0;
        $rkapsemen=0;

        foreach ($sql_semens6000->result_array() as $rowID) {
          $org=$rowID['ORG'];
          $smig_vol_progsemen6000=$rowID['PROGNOSE']+$rowID['REALISASI'];
          $smig_realisasisemen6000=$smig_vol_progsemen6000-$rowID['RKAP'];
          $rkapsemen=$rowID['RKAP'];



          $prod_smigsemen['s'.$org]=array('prognose'=>$rowID['PROGNOSE'],
                        'vol_prog'=>$smig_vol_progsemen6000,
                        'realisasi'=>$smig_realisasisemen6000);
