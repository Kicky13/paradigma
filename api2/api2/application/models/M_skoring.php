...
class M_skoring extends DB_QM {

	/** Private Var for datatable **/
    private $post  = array();
	private $table = "M_BATASAN";
	private $pKey  = "ID_BATASAN";
	private $column_order = array(NULL, 'ID_BATASAN','BATASAN'); //set column for datatable order
    private $column_search = array('JENIS_ASPEK', 'ASPEK', 'KRITERIA', 'BATASAN'); //set column for datatable search 
    private $order = array("a.ID_KRITERIA" => 'ASC'); //default order
	...
	/** Fetch table **/
	public function get(){
		$this->db->select('a.ID_BATASAN, a.ID_KRITERIA, c.ID_ASPEK, d.JENIS_ASPEK, c.ASPEK, d.BOBOT, b.KRITERIA, a.BATASAN, d.ID_JENIS_ASPEK');
		$this->db->from($this->table . ' a');
		$this->db->join('M_KRITERIA b', 'a.ID_KRITERIA = b.ID_KRITERIA', 'left');
		$this->db->join('M_ASPEK c', 'b.ID_ASPEK = c.ID_ASPEK', 'left');
		$this->db->join('M_JENIS_ASPEK d', 'c.ID_JENIS_ASPEK = d.ID_JENIS_ASPEK', 'left');
		$this->db->where('b.HIDDEN IS NULL');
		$this->db->order_by('d.ID_JENIS_ASPEK', 'asc');
	}
	...
	public function get_list($id_jenis_aspek='ALL') {
		$this->get_query();
