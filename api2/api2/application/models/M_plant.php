...
class M_Plant Extends DB_QM {
    ...
    public function datalist($ID_COMPANY = NULL, $ID_PLANT = NULL) {
        ...
        if ($ID_COMPANY)
            $this->db->where_in("a.ID_COMPANY", $ID_COMPANY);
        if ($ID_PLANT)
            $this->db->where("a.ID_PLANT", $ID_PLANT);

        if ($this->USER->ID_COMPANY)
            $this->db->where("a.ID_COMPANY", $this->USER->ID_COMPANY);
        if ($this->USER->ID_PLANT)
            $this->db->where("a.ID_PLANT", $this->USER->ID_PLANT);
