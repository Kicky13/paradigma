...
class D_production extends DB_QM {

	/** Private Var for datatable **/
    private $post  = array();
	private $table = "D_PRODUCTION";
	private $pKey  = "ID_PRODUCTION";
	private $column_order = array(NULL, 'ID_PRODUCTION','ID_PRODUCT','NILAI', 'ID_AREA', 'NO_FIELD'); //set column for datatable order
    private $column_search = array('ID_PRODUCT','NILAI', 'ID_PRODUCTION', 'ID_AREA'); //set column for datatable search 
	...
	public function get_nilai($id_prod='', $no_field=''){
		$this->db->where('ID_PRODUCTION', $id_prod);
		$this->db->where('NO_FIELD', $no_field);
		$query = $this->db->get($this->table);
