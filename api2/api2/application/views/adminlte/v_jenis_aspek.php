        ...
        <div class="box-header">
          <form class="form-inline">
            <div class="input-group input-group-sm" style="width: 150px; ">
              <a href="<?php echo site_url("jenis_aspek/add");?>" type="button" class="btn btn-block btn-primary btn-sm">Create New</a>
            </div>
          </form>
          <hr/>
        </div>
...
<script>
    $(document).ready(function(){
      ...
      "serverSide": true,
      "ajax": {
      "url" : '<?php echo site_url("jenis_aspek/get_list");?>',
      "type": 'post',
      "data": function(data){
    ...
    $(document).on('click',".btEdit",function () {
      var url = '<?php echo site_url("jenis_aspek/edit");?>';
      var data = table.row($(this).parents('tr')).data();
      var id_jenis_aspek = data[0];
      goPost(url, {"ID_JENIS_ASPEK":id_jenis_aspek});
    });

    /** btDelete Click **/
    $(document).on('click',".btDelete",function () {
      var url = '<?php echo site_url("jenis_aspek/remove");?>';
      var data = table.row($(this).parents('tr')).data();
      var id_jenis_aspek = data[0];

      $.confirm({
          title: "Remove Jenis Aspek",
          text: "This company will be removed. Are you sure?",
