        ...
        <div class="box-header">
          <form class="form-inline">
            <div class="input-group input-group-sm" style="width: 150px; ">
              <a href="<?php echo site_url("indikator_hebat/add");?>" type="button" class="btn btn-block btn-primary btn-sm">Create New</a>
            </div>
          </form>
          <hr/>
        </div>
...
<script>
    $(document).ready(function(){
      ...
      "serverSide": true,
      "ajax": {
      "url" : '<?php echo site_url("indikator_hebat/get_list");?>',
      "type": 'post',
      "data": function(data){
    ...
    $(document).on('click',".btEdit",function () {
      var url = '<?php echo site_url("indikator_hebat/edit");?>';
      var data = table.row($(this).parents('tr')).data();
      var id_indikator = data[0];
      goPost(url, {"ID_INDIKATOR":id_indikator});
    });

    /** btDelete Click **/
    $(document).on('click',".btDelete",function () {
      var url = '<?php echo site_url("indikator_hebat/remove");?>';
      var data = table.row($(this).parents('tr')).data();
      var id_indikator = data[0];

      $.confirm({
          title: "Remove Indikator",
          text: "This Indikator be removed. Are you sure?",
