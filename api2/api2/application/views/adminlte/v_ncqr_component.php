				...
				<form class="form-horizontal">
					<div class="box-body">
						<div class="form-group">
							...
							<?php endif; ?>
							<div class="col-sm-2">
								  <BUTTON onclick="document.location.href='<?php echo site_url("ncqr_component/add") ?>/'+$('#ID_PLANT').val();" type="button" class="btn btn-block btn-primary btn-sm">New Configuration</BUTTON>
							</div>
						</div>
...
$(document).ready(function(){
	...
	$("#ID_PLANT").change(function(){
		...
		var url = "<?php echo site_url("ncqr_component/list_config_grouparea/") ?>"+this.value;
		$.get(url,function(data){ // console.log(data);
			var tr = null;
			if(data != null){
				tr = null;
				data.forEach(function(r){
					tr += "<tr><td>"+r.NM_COMPANY+"</td><td>"+r.NM_PLANT+"</td><td>"+r.NM_GROUPAREA+"</td><td><a href='<?php echo base_url("ncqr_component") ?>/edit/"+r.ID_PLANT+"/"+r.ID_GROUPAREA+"' type=button class='btn btn-xs btn-success'>CONFIGURATION</a></td></tr>";	
				});
			}
			else{
				var tr = "<tr><td colspan=5> <i>There is no component configured.</i></td></tr>";
			}
			$("#div_table").html(tr);
