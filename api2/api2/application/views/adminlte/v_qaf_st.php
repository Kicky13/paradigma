...
<section class="content-header">
  <h1>
  Quality Assurance Factor of Time Setting
  <small></small>
  </h1>
</section>
<!-- Main content -->
...
<script>
  $(document).ready(function(){
	...
	function qaf_produksi_st(){ 
		$.post('<?php echo site_url("qaf_report/report_produksi_st");?>', { 
			MONTH: $("#MONTH").val(), 
			YEAR: $("#YEAR").val(), 
	...
	function get_report(r,arr_gr,curr_ik){
		...
		$.post('<?php echo site_url("qaf_report/generate_report/st");?>', { 
			MONTH: $("#MONTH").val(),
			YEAR: $("#YEAR").val(), 
			ID_AREA: r.ID_AREA,
			ID_PLANT: $("#ID_PLANT").val(),
			...
			$.post('<?php echo site_url("qaf_report/get_report_st");?>', { 
				MONTH: $("#MONTH").val(),
				YEAR: $("#YEAR").val(),
    ...
    $("#btLoad").click(function(event){
      ...
      $("#row_qaf_area").html("");
		...
		$.getJSON("<?php echo site_url("qaf_report/setting_time/get_area") ?>/"+$("#ID_PLANT").val()+"/"+$("#ID_PRODUCT").val(),function(gr){
			var ik = 0;
