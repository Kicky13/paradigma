  ...
  <div class="row">
    ...
    <div class="col-md-12">
      ...
      <div class="box box-primary">
        ...
        <!-- /.box-header -->
        <!-- form start -->
        <form role="form" method="POST" action="<?php echo site_url("user/update/".$this->data_user->ID_USER) ?>" >
          <div class="box-body" style="background-color:#FFFAAE;">
            
            <div class="form-group">
              <div class="col-sm-4 clearfix">
                <label>FULLNAME </label>
                <input id="fullName" type="text" class="form-control" name="FULLNAME" placeholder="Full Name" value="<?php echo $this->data_user->FULLNAME ?>" >
              </div>
            </div>
            <div class="form-group">
              <div class="col-sm-4 clearfix">
                <label>USERNAME </label>
                <input id="uName" type="text" class="form-control" name="USERNAME" placeholder="User Name" value="<?php echo $this->data_user->USERNAME ?>">
              </div>
            </div>
            ...
            <div class="form-group">
              <div class="col-sm-4 clearfix">
                ...
                <select class="form-control select2" id="ID_USERGROUP" NAME="ID_USERGROUP" >
                  ...
                  <option value="<?php echo $usergroup->ID_USERGROUP ?>" <?php echo ($this->data_user->ID_USERGROUP == $usergroup->ID_USERGROUP)?"SELECTED":""; ?> ><?php echo $usergroup->NM_USERGROUP ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
            </div>
...
$(function(){
	...
	$("#ID_COMPANY").change(function(){
		...
		$.getJSON(url,function(data){
			$("#ID_PLANT").empty();
			$("#ID_PLANT").append("<option value=''></option>");
			data.forEach(function(r){
				$("#ID_PLANT").append("<option value='"+r.ID_PLANT+"' "+((USER_PLANT==r.ID_PLANT)?"SELECTED":"")+" >"+r.NM_PLANT+"</option>");
			});
			$("#ID_PLANT").change();
		});
	});
	...
	$("#ID_PLANT").change(function(){
		...
		$.getJSON(url,function(data){
			$("#ID_AREA").empty();
			$("#ID_AREA").append("<option value=''></option>");
			data.forEach(function(r){
				$("#ID_AREA").append("<option value='"+r.ID_AREA+"' "+((USER_AREA==r.ID_AREA)?"SELECTED":"")+">"+r.NM_AREA+"</option>");
			});
		//	$("#ID_AREA").change();
		});		
	});
