...
<section class="content-header">
  <h1>
  Quality Assurance Factor of Compressive Strength
  <small></small>
  </h1>
</section>
<!-- Main content -->
...
<script>
  $(document).ready(function(){
	...
	$("#ID_PLANT").change(function(){
		...
		var url = "<?php echo site_url("qaf_report/async_list_grouparea/") ?>"+this.value;
	...
	function qaf_produksi_cs(){ 
		$.post('<?php echo site_url("qaf_report/report_produksi_cs");?>', { 
			MONTH: $("#MONTH").val(), 
			YEAR: $("#YEAR").val(), 
	...
	function get_report(r,arr_gr,curr_ik){
		...
		$.post('<?php echo site_url("qaf_report/generate_report/cs");?>', { 
			MONTH: $("#MONTH").val(),
			YEAR: $("#YEAR").val(), 
			ID_AREA: r.ID_AREA,
			ID_PLANT: $("#ID_PLANT").val(),
			...
			$.post('<?php echo site_url("qaf_report/get_report_cs");?>', { 
				MONTH: $("#MONTH").val(),
				YEAR: $("#YEAR").val(),
    ...
    $("#btLoad").click(function(event){
      ...
      $("#row_qaf_area").html("");
		...
		$.getJSON("<?php echo site_url("qaf_report/compressive_strength/get_area") ?>/"+$("#ID_PLANT").val()+"/"+$("#ID_PRODUCT").val(),function(gr){
			var ik = 0;
