            ...
            <div class="form-group row" id="row_qaf_area">
				...
				<div class="col-sm-6 col-sm-12">
					<div class="box-body table-responsive no-padding">
						...
						<TH COLSPAN=8><?php echo $this->NM_PRODUCT ?></TH>
						</tr>					
						<tr>
							<th>3D</th>
							<th>7D</th>
...
<script>
  $(document).ready(function(){
	...
	$("#ID_PLANT").change(function(){
		...
		var url = "<?php echo site_url("qaf_report/async_list_product_qaf") ?>/"+$("#ID_PLANT").val()+"/1";
	...
	function qaf_company(r,arr_gr,curr_ik){
		...
		$.post('<?php echo site_url("qaf_report/generate_company_report");?>', { 
			MONTH: $("#MONTH").val(), 
			YEAR: $("#YEAR").val(),
			ID_COMPANY: r.ID_COMPANY,
			ID_GROUPAREA: 1
		}, function (nres) {
			$.post('<?php echo site_url("qaf_report/get_company_report");?>', { 
				MONTH: $("#MONTH").val(), 
				YEAR: $("#YEAR").val(), 
				ID_COMPANY: r.ID_COMPANY,
				ID_GROUPAREA: 1
	...
	function qaf_produksi(){ 
		$.post('<?php echo site_url("qaf_report/report_produksi");?>', { 
			MONTH: $("#MONTH").val(), 
			YEAR: $("#YEAR").val(), 
	...
	function get_report(r,arr_gr,curr_ik){
		...
		$.post('<?php echo site_url("qaf_report/generate_report");?>', { 
			MONTH: $("#MONTH").val(), 
			YEAR: $("#YEAR").val(),
			ID_PRODUCT: $("#ID_PRODUCT").val(), 
			ID_AREA: r.ID_AREA,
			...
			$.post('<?php echo site_url("qaf_report/get_report");?>', { 
				MONTH: $("#MONTH").val(), 
				YEAR: $("#YEAR").val(), 
    ...
    $("#btLoad").click(function(event){
      ...
      $("#row_qaf_area").html("");
		if($("#ID_COMPANY").val() == 7){ //SMIG
			//get company list
			$.getJSON("<?php echo site_url("qaf_report/get_company_list") ?>",function(gr){
				var ik = 0;
			...
			$.getJSON("<?php echo site_url("qaf_report/get_area_by_group") ?>/"+$("#ID_PLANT").val()+"/1/"+$("#ID_PRODUCT").val(),function(gr){
				var ik = 0;
				var arr_gr = new Array();
