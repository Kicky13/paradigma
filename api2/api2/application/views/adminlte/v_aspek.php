        ...
        <div class="box-header">
          ...
          <?php if($this->PERM_WRITE): ?>
            <div class="input-group input-group-sm" style="width: 150px; ">
              <a href="<?php echo site_url("aspek/add");?>" type="button" class="btn btn-block btn-primary btn-sm">Create New</a>
            </div>
          <?PHP endif; ?>
          </form>
          <hr/>
...
<script>
    $(document).ready(function(){
      ...
      "serverSide": true,
      "ajax": {
      "url" : '<?php echo site_url("aspek/get_list");?>',
      "type": 'post',
      "data": function(data){
    ...
    $(document).on('click',".btEdit",function () {
      var url = '<?php echo site_url("aspek/edit");?>';
      var data = table.row($(this).parents('tr')).data();
      var id_aspek = data[0];
      goPost(url, {"ID_ASPEK":id_aspek});
    });

    /** btDelete Click **/
    $(document).on('click',".btDelete",function () {
      var url = '<?php echo site_url("aspek/remove");?>';
      var data = table.row($(this).parents('tr')).data();
      var id_aspek = data[0];

      $.confirm({
          title: "Remove Jenis Aspek",
          text: "This aspek will be removed. Are you sure?",
