                  ...
                  <div class="box-header">
                    <form class="form-inline">
                      ...
                      <div class="input-group input-group-sm" style="width: 150px; ">
                        <a id="btAdd" href="<?php echo site_url("menu/add");?>" type="button" class="btn btn-block btn-primary btn-sm">Create New</a>
                      </div>
                      <?PHP endif; ?>
                    </form>
                    <hr/>
...
$(document).ready(function(){
  ...
  var table = $("#dt_tables").DataTable({
	...
	$("#ID_GROUPMENU").change(function(){
		/*var trm = $(".trm");
		for(i=0; i<trm.length; i++){
			$(trm[i]).css("display",(($(trm[i]).attr("ID_GROUPMENU") == this.value)?"":"none"));
		}*/
 		table.ajax.reload();
		$("a#btAdd").attr("href", '<?php echo site_url("menu/add");?>' + '/' + this.value);
    ...
    $(document).on('click',".btEdit",function () {
      ...
      var url = '<?php echo site_url("menu/edit/");?>' + data[0];
    ...
    $(document).on('click',".btDelete",function () {
      var data = table.row($(this).parents('tr')).data();
      var url = '<?php echo site_url("menu/delete/");?>' + data[0];
